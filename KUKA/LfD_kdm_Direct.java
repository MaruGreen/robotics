 ...
 * @see #dispose()
 */
public class LfD_kdm_Direct extends RoboticsAPIApplication
{
	...
	public static double getSqrSumSqrt(double[] arr)
	{
		double result = 0.0;
	...
	private void moveToInitialPosition()
    {
		...
		demo = false;
		// traditional method to set position in joint space
        ...
        lbr_iiwa_14_R820_1.move(ptp(Math.toRadians(54.02), Math.toRadians(58.01), Math.toRadians(-8.38), Math.toRadians(-62.80),
        		Math.toRadians(126.15), Math.toRadians(50.90), Math.toRadians(25.67)).setJointVelocityRel(0.3));
	...
	public void run()
	{
	    ...
	    try
	    {
			...
			//FingerTip.setPosReq(0) ; // open the grip
	        //ThreadUtil.milliSleep(2000); // wait for 2 seconds
			...
			if (origin)
			{
				FingerTip.setActReq(11); // finger touches fingers
			...
			while(!done)
			{
				// using setDestination, update is called, not need to manually call
		      ...
		      //if grip changes
				...
				else
				{
					...
					{
						// get the goal coordinate from the socket to goalFrame
				...
				currentFrame = lbr_iiwa_14_R820_1.getCurrentCartesianPosition(ToolGripper.getFrame("/BasePad"),getApplicationData().getFrame("/tableOrigin"));
...
//              System.out.println("sent Packet!");
	...
	{
		LfD_kdm_Direct app = new LfD_kdm_Direct();
		app.runApplication();
	}
}
